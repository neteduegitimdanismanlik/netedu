...
export default function CAS() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      else { setUser(data.user); await loadEvents() }
    })
  }, [])
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="What is this event about? What will participants do?" className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-indigo-500 resize-none h-20" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input value={fee} onChange={e => setFee(e.target.value)} placeholder="Free / 200 TL / etc." className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-indigo-500" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3">
                <p className="text-xs text-yellow-700">After the event, you will need to upload photos and participant list for verification. Only then will it count toward participants' portfolios.</p>
              </div>
          ...
          <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl p-6 w-full max-w-md">
              ...
              <textarea value={applyMessage} onChange={e => setApplyMessage(e.target.value)}
                placeholder="Why do you want to join? What can you contribute?" className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-indigo-500 resize-none h-24 mb-4" />
              <div className="flex gap-3">
            ...
            {filtered.map((event, i) => (
              <div key={i} className="bg-white rounded-2xl border border-gray-100 p-6 hover:border-indigo-200 transition-colors">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <span className={`text-xs px-2 py-1 rounded-full border ${typeColors[event.event_type] || 'bg-gray-50 text-gray-500'}`}>
                        {event.event_type === 'institution' ? '🏢 Institution' : '👤 Student'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {event.created_by !== user?.id && (
                  <button onClick={() => setApplying(event.id)}
                    className="w-full bg-indigo-900 text-white py-2.5 rounded-xl text-sm font-medium hover:bg-indigo-800">
                    Apply to join →
                  </button>
                )}
                {event.created_by === user?.id && (
